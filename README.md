...
* nginx serves **one certificate per server block** (it can't reliably pick a
  cert per-SNI via variables). A site with its own real cert gets its own
  block (see `nginx/conf.d/php-site.conf` for the split). Blocks without a
  real cert serve the self-signed multi-SAN fallback in
  `env/letsencrypt/live/test-all` (generated by `gen-test-certs.sh` in test
  mode, or the deploy.sh bootstrap cert); `deploy.sh` seeds a placeholder into
  each `live/<name>` so a not-yet-issued cert degrades to the fallback instead
