...
Everything needed to rebuild lives in this repo. A fresh host = `git clone` +
...
| Script | What it does |
|---|---|
| `scripts/bootstrap.sh` | One-time host setup: installs podman/podman-compose/git/openssl/sshfs, creates bind-mount dirs. |
| `scripts/deploy.sh` | `git pull` (skipped if no remote) -> build nginx image -> `nginx -t` -> `podman compose up -d --build` -> install systemd units. |
